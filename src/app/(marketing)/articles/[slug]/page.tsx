...
export async function generateStaticParams() {
  const sanitySlug: { slug: string }[] = await sanityClient.fetch(allPostSlugsQuery);
  const all = new Map(sanitySlug.map(s => [s.slug, true]));
  all.set('how-ocd-therapy-works', true);
  return Array.from(all.keys()).map(slug => ({ slug }));
}
...
export default async function ArticlePage({ params }: Props) {
  ...
  // Hardcoded OCD article — handled below
  if (slug !== 'how-ocd-therapy-works') {
    const post = await sanityClient.fetch(postBySlugQuery, { slug });
    if (!post) notFound();
      ...
      <>
        ...
        >
          ...
          <div className="relative z-10 max-w-4xl mx-auto">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-cream/70">
              <span>By <Link href="/about" className="text-cream/90 h-hover:text-cream h-can:transition-colors underline underline-offset-2">{post.author ?? 'Owen Lynch'}</Link></span>
              {publishedDate && <><span aria-hidden="true">·</span><time dateTime={post.publishedAt}>{publishedDate}</time></>}
            </div>
          </div>
        </section>
        ...
        >
          <div className="max-w-2xl mx-auto text-center">
            <h2 id="post-cta-heading" className="font-heading font-light text-3xl sm:text-4xl text-cream mb-6">
              Could therapy help?
            </h2>
